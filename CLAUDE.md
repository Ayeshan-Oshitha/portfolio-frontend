...
**Content/presentation split.** This is the central convention. `src/client/data/*.ts` exports frozen, `readonly`-typed constants (`HERO_DATA`, `ALL_PROJECTS`, `TECHNOLOGY_CATEGORIES`, …); components in `src/client/components/**` import those constants directly rather than receiving them as props. Every content shape is declared as an interface in `src/client/types.ts`. To change copy, edit the data file. To add a content type, add the interface to `types.ts` first, then a data module, then components.

**Routing.** `src/main.tsx` mounts `RouterProvider` with the router in `src/routes/routes.tsx`. All routes are children of `ClientLayout` (`Header` / `<Outlet/>` / `Footer`). `src/App.tsx` is vestigial — it renders `Home` but is not in the route tree; do not add anything to it.

**Component directory naming.** Two parallel groups under `src/client/components/`:
